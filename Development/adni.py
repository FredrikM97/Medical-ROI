...
class ADNI_PROPERTIES:
    columns:list = [
            ...
            'subject.study.series.modality', #PT
            'subject.study.imagingProtocol.description', #ADNI_Brain_PET__Raw_FDG
            'subject.study.series.dateAcquiredPrecise', #20120120110538472
            'image_nbr',#374
            'series',#S137674
    ...
    files:list = []
    meta:list = []
    category_cols = ['subject.researchGroup','subject.subjectIdentifier','subject.study.imagingProtocol.imageUID','image_nbr']
    path = ADNI_PATHS()
...
class ADNI(ADNI_PROPERTIES):
    ...
    def load_files(self,path=None, columns=None, show_output=True) -> iter:
        ...
        (path,columns,func) = (path,columns,self.info_from_custom_filename) if path and columns else (self.path.raw,self.columns,self.info_from_raw_filename)
        #if not (path and columns): path=self.raw_image_dir; columns=self.columns
    ...
    def get_files(self, path=None,columns=None) -> iter:
        ...
        #files = self.load_files(path=path) if path is not None else self.files    
    ...
    def save_to_category(self,output_df, path:dict=None, show_output=True)->None:
        ...
        def inner(row):
            filename = f"{'#'.join([row[p] for p in self.category_cols])}.nii"
