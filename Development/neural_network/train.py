...
class Agent:
    def __init__(self, config_name:str, export:bool=True):
        ...
        # Setup visualisation
        self.visualizer = Visualizer(self.config['visualization_params'])   # create a visualizer that displays images and plots
    ...
    def fit(self):
        ...
        for epoch in range(starting_epoch, num_epochs):
            ...
            for step, data in enumerate(self.train_dataset,0):  # inner loop within one epoch
                print("type", data[0].type(), data[1].type())
                self.train_step(data)
                print(f"Step: {step}", sep="\r")
            
            # Evaluate model
            self.model.eval()
            for step, data in enumerate(self.val_dataset):
                self.test_step(data)
    ...
    def train_step(self, data):
        "Enumerate one item of mini batch"
