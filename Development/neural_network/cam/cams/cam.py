...
class _CAM:
    ...
    def compute_cams(self, class_idx: int, scores: Optional[Tensor] = None, normalized: bool = True) -> Tensor:
        ...
        print("View",weights.view(*weights.shape, 1,1).shape, "Hook Squeese", self.hook_a.squeeze(0).shape, "weights", weights.shape, "Hook", self.hook_a.shape)
        batch_cams = (weights.view(*weights.shape) * self.hook_a.squeeze(0)).sum(0) 
        #batch_cams = (weights.view(*weights.shape, 1,1) * self.hook_a.squeeze(0)).nansum(0)  # type: ignore[union-attr]
        #print("Print1",batch_cams.shape)
        if self._relu:
            batch_cams = F.relu(batch_cams, inplace=True)
        #print("Print2",batch_cams)
        # Normalize the CAM
        if normalized:
            batch_cams = self._normalize(batch_cams)
        print("Print3",batch_cams)
        return batch_cams
...
class SSCAM(ScoreCAM):
    ...
    def _get_weights(self, class_idx: int, scores: Optional[Tensor] = None) -> Tensor:
        ...
        for _idx in range(self.num_samples):
            noisy_m = self._input * (upsampled_a + self._distrib.sample(self._input.size()).to(device=self._input.device))
