...
class SmoothGradCAMpp(_GradCAM):
    ...
    def _get_weights(self, class_idx: int, scores: Optional[Tensor] = None) -> Tensor:
        ...
        self.hook_a: Tensor
        self.hook_g: Tensor
        #print("what is dis",self.hook_g.pow(2).shape, self.hook_g.shape)
        ...
        for _idx in range(self.num_samples):
            ...
            # Forward & Backward
            print("Shape of input!", noisy_input.shape)
        ...
        print("What is ahlpha??",alpha.shape, self.hook_g.squeeze(0).shape, torch.relu(self.hook_g.squeeze(0)).shape,alpha.squeeze_(0).mul_(torch.relu(self.hook_g.squeeze(0))).shape)
        # Apply pixel coefficient in each weight
        return alpha.squeeze_(0).mul_(torch.relu(self.hook_g.squeeze(0))).sum(dim=(1,2))
