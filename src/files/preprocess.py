...
def normalize(x:"Union[tensor,ndarray]") -> "Union[tensor,ndarray]":
...
def batchisize_to_5D(x:"Union[tensor,ndarray]") -> "Union[tensor,ndarray]":
...
def folder2labels(images:list, classes:dict) -> "ndarray": #, delimiter:str
...
def mask_threshold(image:"ndarray", threshold:float) -> None:
...
def set_outside_of_scan_pixels_2_zero(image:"ndarray", mask:"ndarray") -> "ndarray":
...
def image2axial(image:"ndarray") -> "ndarray":
    ...
    """
    if not isinstance(image, "ndarray"): raise TypeError(f"Expected 'ndarray'. Got: {type(image)}")
...
def preprocess_image(image:"ndarray", input_shape:Tuple=(79,95,79), normalized=True) -> Tensor:
...
def to_grid(image:"ndarray", max_num_slices:int=None,pad_value:float=0.5, nrow:int=10, padding:int=2) -> Tensor:
